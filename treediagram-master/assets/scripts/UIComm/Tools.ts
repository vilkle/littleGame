...
export class Tools  {
   /**
    ...
    //矩形碰撞检测(0.5,0.5锚点)
    public static isCollisionWithRect(position1:cc.Vec2,width1:number,height1:number,
                                      position2:cc.Vec2,width2:number,height2:number){
        if(Math.abs(position1.x-position2.x)<=width1/2+width2/2
            &&Math.abs(position1.y-position2.y)<=height1/2+height2/2){
                return true;
        }
        return false;
    }
     ...
     /* 获取两点距离 */
     public static GetDistance(startPose:cc.Vec2,EndStart:cc.Vec2){

        return Math.sqrt( (startPose.x - EndStart.x) *(startPose.x - EndStart.x) +
        (startPose.y - EndStart.y) *(startPose.y - EndStart.y) );
    }
